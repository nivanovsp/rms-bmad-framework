...
mlda_protocol:
  paradigm:
    ...
    - Stories reference DOC-IDs as entry points for developers to navigate
    - SM creates stories that point into the knowledge graph, not self-contained specs
  activation:
  ...
  navigation:
    ...
    - Follow depends-on relationships to understand requirements context
    - Use *related to discover connected documents
    - Use *context to see gathered context summary
    - Default depth limit 3 for SM (story-focused)
  story_creation:
    - Stories MUST reference relevant DOC-IDs as entry points
    - Include DOC-IDs in story so dev agent can navigate the knowledge graph
    - Stories are entry points, not comprehensive specs
